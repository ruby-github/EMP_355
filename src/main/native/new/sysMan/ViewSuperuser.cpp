...
void ViewSuperuser::CreateWindow() {
  ...
  for (i = 0; i < size; i ++) {
      ...
      ProbeMan::GetInstance()->VerifyProbeName(probeName[i].c_str(), newProbeName);
  ...
  m_treeviewProbe = CreateProbeTreeview();
...
GtkWidget* ViewSuperuser::CreateProbeTreeview() {
  GtkWidget *treeview;
  GtkTreeModel *model;
