...
enum EDCMReturnStatus {
  DCMSUCCESS,             DCMNODEFALUTSERVER,   DCMCONNECTIONFAILURE, DCMNONSUPPORTSTUDYROOT,   DCMINVALIDVALUE,
  DCMINVALIDQRDESTSERVER, DCMRETRIEVEFAILURE,   DCMSTUDYEXISTED,      DCMINVALIDSTORAGEDEVICE,  DCMEXPORTFAILURE,
  DCMNOENOUGHSPACE,       DCMNONEXISTDICOMDIR,  DCMNONENTIREDICOMDIR, DCMIMPORTFAILURE
};
...
// nPos current position of progress, the total value is 100
typedef void (*PROGRESSSTATUS)(int nPos);

// return studyNo
typedef unsigned int (*GetDCMStudyElement)(DCMSTUDYELEMENT element);

// progressPos the progress pos of retrieve current image or/and sr
