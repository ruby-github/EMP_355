...
// signal handle
    ...
    bool SelectNext(GtkWidget *tree_view, GtkTreeModel *model, GtkTreeSelection *selection, GtkTreeIter *iter);

    // signal connect
    static void HandleExamTypeBtnClicked(GtkWidget *widget, GdkEventButton *event, TreeMenu *data) {
        data->BtnExamTypeClicked(widget, event);
    }
