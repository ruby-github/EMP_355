...
//	void BtnDeselectClicked(GtkButton *button);
    ...
    //signal connect
    static gboolean on_window_delete_event(GtkWidget *widget, GdkEvent *event, ViewUdiskDataSelect* data) {
        return data->WindowDeleteEvent(widget, event);
    }
    static void on_data_toggled(GtkCellRendererToggle *cell, gchar *path_str, ViewUdiskDataSelect* data) {
        data->ToggleData(cell, path_str);
    }
    static void on_root_selection_changed(GtkTreeSelection *selction, ViewUdiskDataSelect* data) {
        data->RootSelectionChanged(selction);
    }
    static void on_button_ok_clicked(GtkButton *button, ViewUdiskDataSelect* data) {
        data->BtnOKClicked(button);
    }
    static void on_button_cancel_clicked(GtkButton *button, ViewUdiskDataSelect* data) {
