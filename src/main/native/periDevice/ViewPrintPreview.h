...
#define IMG_SCALE_WIDTH		(440)//(IMG_W*IMAGE_SCALE)
    ...
    static gboolean HandleExposeImage(GtkWidget *widget, GdkEventExpose *event, ViewPrintPreview *data) {
        data->ExposeImage(widget, event);
        return FALSE;
    }
    static void HandleClickPrintButton(GtkButton *button, ViewPrintPreview *data) {
        data->ClickedPrintButton(button);
    }
...
// signal handle
    ...
    static gboolean HandleDeleteEvent(GtkWidget *widget, GdkEvent *event, ViewPrintPreview *data) {
        data->BtnExitClicked(NULL);
        return FALSE;
    }

    static gboolean HandleDeleteWinEvent(GtkWidget *widget, GdkEvent *event, ViewPrintPreview *data) {
        data->BtnExitWinClicked(NULL);
        return FALSE;
    }

    static void on_button_print_clicked(GtkButton *button, ViewPrintPreview *data) {
        data->BtnPrintClicked(button);
    }
    static void on_button_exit_clicked(GtkButton *button, ViewPrintPreview *data) {
        data->BtnExitClicked(button);
    }
    static void on_button_setting_clicked(GtkButton *button, ViewPrintPreview *data) {
        data->BtnSettingClicked(button);
    }

//2012.10.10 lihuamei
    static gboolean HandlePrintPreviewAreaConfigure(GtkWidget *widget, GdkEventConfigure *event, ViewPrintPreview *data) {
        data->PrintAreaConfigure(widget, event);
        return FALSE;
    }

    static gboolean HandlePrintPreviewAreaExpose(GtkWidget *widget, GdkEventExpose *event, ViewPrintPreview *data) {
        data->PrintAreaExpose(widget, event);
        return FALSE;
    }

    static void button_print_clicked(GtkButton *button, ViewPrintPreview *data) {
        data->BtnPrintWinClicked(button);
    }
    static void button_exit_clicked(GtkButton *button, ViewPrintPreview *data) {
        data->BtnExitWinClicked(button);
    }
    static void button_setting_clicked(GtkButton *button, ViewPrintPreview *data) {
        data->BtnSettingWinClicked(button);
    }
    static void on_auto_radio_button_toggled(GtkToggleButton *togglebutton, ViewPrintPreview *data) {
        data->AutoRadioToggled(togglebutton);
    }
    static void on_orignial_radio_button_toggled(GtkToggleButton *togglebutton, ViewPrintPreview *data) {
