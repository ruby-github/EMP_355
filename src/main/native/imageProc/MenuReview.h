...
public:
    ...
    const char* GetReadFrm() {
        return m_readFrm.c_str();
    }
    ...
    ReviewType GetCurType() {
        return m_reviewType;
    }
    void SetCurType(ReviewType type) {
        m_reviewType = type;
    }
    void SetLimit(guchar limit) {
        m_limit = limit;
    }
    guchar GetLimit() {
        return m_limit;
    ...
    bool OnlyOneVideoDemo(void) {
        return ((m_reviewType == REVIEW_VIDEO)&&(1 == m_numTotal));
    }
    GtkWidget* CurrentPreview() {
        return m_cur_btn;
    }
...
private:
    ...
    static void HandleChkBtnClicked(GtkButton *button, MenuReview *data) {
        data->ChkBtnClicked(button);
    }
    static void HandleChgShowClicked(GtkButton *button, MenuReview *data) {
        data->ChangeShowClicked(button);
    }
    static void HandleSendClicked(GtkButton *button, MenuReview *data) {
        data->SendClicked(button);
    }
    static void HandleUnSelectClicked(GtkButton *button, MenuReview *data) {
        data->UnSelectClicked(button);
    }
    static void HandleSelAllClicked(GtkButton *button, MenuReview *data) {
        data->SelAllClicked(button);
    }
    static void HandleDelSelClicked(GtkButton *button, MenuReview *data) {
        data->DelSelClicked(button);
    }
    static void HandlePageUpClicked(GtkButton *button, MenuReview *data) {
        data->PageUpClicked(button);
    }
    static void HandlePageDownClicked(GtkButton *button, MenuReview *data) {
        data->PageDownClicked(button);
    }
};
